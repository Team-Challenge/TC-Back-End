...
def test_signup_1(client):
    """The test returns successfull response"""
    # Given
    payload = {
...
def test_empty_request_delete_profile_photo(client):
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZEDresponse = client.get(f'/accounts/verify/{bad_token}')
    assert response.status_code == status.HTTP_404_NOT_FOUND
