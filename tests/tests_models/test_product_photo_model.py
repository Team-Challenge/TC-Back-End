...
def test_add_product_photo_1(session):
    ...
    with patch("werkzeug.datastructures.file_storage.FileStorage.save") as func:
        photos.append(ProductPhoto.add_product_photo(
            data.user.id, data.product.id, image, True))
        photos.append(ProductPhoto.add_product_photo(
            data.user.id, data.product.id, image, False))
        photos.append(ProductPhoto.add_product_photo(
            data.user.id, data.product.id, image, False))
        photos.append(ProductPhoto.add_product_photo(
            data.user.id, data.product.id, image, False))
    ...
    added = ProductPhoto.query.filter_by(
        product_detail_id=data.detail.id).all()
    assert added[0].id > 0
...
def test_add_product_photo_3(session):
    ...
    with pytest.raises(ProductPhotoLimitError):
        with patch("werkzeug.datastructures.file_storage.FileStorage.save") as func:
            ProductPhoto.add_product_photo(
                data.user.id, data.product.id, file_storage, True)
            ProductPhoto.add_product_photo(
                data.user.id, data.product.id, file_storage, False)
            ProductPhoto.add_product_photo(
                data.user.id, data.product.id, file_storage, False)
            ProductPhoto.add_product_photo(
                data.user.id, data.product.id, file_storage, False)
...
def test_get_num_photos(session):
    ...
    # Then
    assert ProductPhoto.get_num_photos_by_product_detail_id(
        data.detail.id) == 0
